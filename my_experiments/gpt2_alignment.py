...
def causal_model_1():
    ...
    functions = {"X":FILLER, "Y":FILLER, "Z":FILLER, 
                "P": lambda x, y: (
                    print(tokenizer.decode(x[0][0], skip_special_tokens=True).split()),
                    int(tokenizer.decode(x[0][0], skip_special_tokens=True).split()) + int(tokenizer.decode(y[0][0], skip_special_tokens=True).split())
                ),
                "O": lambda x,y: int(tokenizer.decode(x[0][0], skip_special_tokens=True).split()) + int(tokenizer.decode(y[0][0], skip_special_tokens=True).split())}
