...
def main():
    ...
    for id in [1,2,3]:
        ...
        for token in range(6):
            ...
            plt.scatter(range(model_config.n_layer), values)
            plt.plot(range(model_config.n_layer), values, label=f'{token}')
            ...
            plt.legend()
            plt.tight_layout()
            plt.savefig(f'{id}_accuracy_per_layer_tkn_{token}.png')
            plt.close()
